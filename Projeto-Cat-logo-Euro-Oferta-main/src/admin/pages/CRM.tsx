      ...
      <div className="flex justify-between items-start z-10 w-full relative">
         <div className="flex-1 min-w-0 pr-2 flex gap-3">
           ...
           <div className="flex flex-col gap-0.5 min-w-0">
             ...
             <a 
               href={getWhatsAppLink(lead.phone, `Olá ${lead.name}, aqui é da Euro Oferta! Agradecemos o seu pedido. Um de nossos consultores já vai dar o atendimento para finalizar o seu pedido.`)}
             target="_blank"
             rel="noreferrer"
...
export const LeadDetailsModal = ({ lead, onClose }: { lead: Lead, onClose: () => void }) => {
    ...
    return {
      abordagem: [
        { 
          ...
          text: `Olá, ${nome}! Tudo bem? Vi que você chegou até nós via ${origem}. Me chamo ${vendedor} e sou seu consultor pessoal aqui na Euro Oferta! Como posso te ajudar na escolha do seu produto ideal hoje?`
        },
        { 
      ...
      objecoes: [
        { 
          ...
          text: `Compreendo perfeitamente, ${nome}. O valor reflete a nossa qualidade Premium estrutural, acabamento exclusivo em todas as etapas e garantia de satisfação Euro Oferta. Além disso, conseguimos facilidades incríveis de parcelamento ou desconto à vista no Pix. Que tal fazermos uma simulação sob medida pro seu orçamento hoje?`
        },
        { 
          label: 'Vou Pensar / Cônjuge', 
          text: `Sem problemas, ${nome}! É importante decidir com calma. Para te ajudar a avaliar em família, posso te enviar um catálogo de fotos detalhadas com as especificações? Ah, se fecharmos hoje, consigo adicionar um brinde especial da Euro Oferta para você!`
        }
      ],
      fechamento: [
        { 
          label: 'Gatilho de Escassez', 
          text: `Oi, ${nome}! Estou passando para te avisar que as unidades deste lote exclusivo já estão quase esgotas na Euro Oferta. Gostaria de reservar o seu agora mesmo antes que acabe? O envio pode ser feito de forma prioritária!`
        },
        { 
      ...
      posvendas: [
        { 
          ...
          text: `Olá, ${nome}! Passando para saber se o seu pedido chegou com total segurança e o que você achou dos produtos! Sua satisfação é o nosso maior objetivo aqui na Euro Oferta.`
        },
        { 
  ...
  const handleTriggerAnalysis = () => {
    ...
    setTimeout(() => {
      ...
      if (vip !== 'Nenhum' || gasto > 1000) {
        ...
        script = `Olá, ${nome}! Como você é um de nossos clientes VIPs favoritos na Euro Oferta, separei por aqui duas alternativas de alto nível que acabaram de desembarcar em nosso estoque restrito. Gostaria que eu mostrasse os detalhes técnicos por chamada de vídeo ou fotos especiais?`;
  ...
  const fastTagsList = React.useMemo(() => {
    ...
    const vendedor = formData.assignee !== 'Sem Responsável' ? formData.assignee : 'seu Consultor Euro Oferta';
    const origem = formData.source || 'Manual';
    const vip = lead.vipLevel && lead.vipLevel !== 'Nenhum' ? lead.vipLevel : 'Especial';
    const valor = Number(formData.estimatedValue || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
    const dynamicLink = `https://eurooferta.com.br/pay/${lead.id}`;
    ...
    return [
      {
        ...
        template: `Olá, ${nome}! Tudo bem? Vi que você chegou até nós via ${origem}. Me chamo ${vendedor} e sou seu consultor pessoal aqui na Euro Oferta! Como posso te ajudar na escolha do seu produto ideal hoje?`
      },
      {
        id: 'retomada-quente',
        label: 'Retomada Leve 📞',
        ...
        template: `Oi, ${nome}! Tudo bem? Passando para te desejar um excelente dia! Ficou com alguma dúvida sobre o modelo de Euro Ofertas que estávamos vendo? Caso já tenha decidido, consigo agilizar seu envio prioritário hoje mesmo!`
      },
      {
        id: 'oferta-pix',
        label: 'Oferta no Pix ⚡',
        ...
        template: `Notícia boa, ${nome}! Consegui com o gerente Euro Oferta um desconto adicional exclusivo de 10% para pagamento à vista via Pix. O seu pedido fica especial em R$ ${valor} e com frete expresso por nossa conta. Vamos aproveitar hoje?`
      },
      {
        ...
        template: `Compreendo perfeitamente o receio, ${nome}! É super compreensível comprar online com cuidado. Na Euro Oferta nós garantimos 100% de satisfação: emitimos nota fiscal corporativa, seu pacote tem seguro de transporte total e, se o tamanho não ficar perfeito, a primeira troca é totalmente grátis!`
      },
      {
        id: 'escassez-lote',
      ...
      },
      {
        ...
        template: `Perfeito, ${nome}! Tudo reservado por aqui em seu nome. Para concluir o seu pedido de forma segura e imediata, é só clicar no link oficial da Euro Oferta: ${dynamicLink} . Assim que aprovar, já despachamos!`
      },
      {
        id: 'rastreio-envio',
        ...
        template: `Boa notícia, ${nome}! O seu pedido Euro Oferta já foi embalado com carinho e despachado. O código de rastreamento oficial brasileiro para acompanhar o trajeto em tempo real é: ${trackingCode}. Qualquer dúvida é só me acionar!`
      },
      {
            ...
            <div className="space-y-4">
              ...
              {isEditing ? (
                ...
                <div className="space-y-3">
                  ...
                  <a href={getWhatsAppLink(formData.phone, `Olá ${lead.name}, aqui é da Euro Oferta! Agradecemos o seu pedido. Um de nossos consultores já vai dar o atendimento para finalizar o seu pedido.`, whatsappRemoveNine)} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-sm text-[#0F172A] hover:text-green-600 transition-colors group p-3 bg-gray-50 rounded-xl border border-gray-100">
                              ...
                              <div className={`border rounded-xl p-4 transition-all ${sch.completed ? 'bg-gray-50/80 border-gray-200/60 opacity-80' : isOverdue ? 'bg-red-50/50 border-red-200/80 shadow-[0_2px_10px_rgba(239,68,68,0.05)]' : 'bg-white border-amber-200/50 shadow-[0_2px_10px_rgba(245,158,11,0.05)]'}`}>
                                ...
                                <div className="flex gap-3">
                                  ...
                                  {!sch.completed && sch.type === 'whatsapp' && (
                                     <a href={getWhatsAppLink(lead.phone, `Olá ${lead.name}, tudo bem? Aqui é da Euro Oferta.`, whatsappRemoveNine)} target="_blank" rel="noreferrer" className="shrink-0 p-2 text-green-600 bg-green-50 hover:bg-green-500 hover:text-white rounded-lg transition-colors border border-green-200/50 self-start" title="Abrir WhatsApp">
