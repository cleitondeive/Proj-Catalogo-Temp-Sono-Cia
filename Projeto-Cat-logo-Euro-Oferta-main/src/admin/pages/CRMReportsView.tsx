...
export const CRMReportsView = ({ leads }: { leads: Lead[] }) => {
  ...
  const exportIndividualSellerPDF = (dev: any) => {
    try {
      ...
      doc.setFontSize(20);
      doc.text("EURO OFERTA PREMIUM", 18, 28);
      ...
      const diagnosisText = dev.ganhas > 0 
        ? `${dev.name} demonstra uma taxa de conversao solida de ${individualCxRate}%, conduzindo de forma ativa as negociacoes com os clientes da Euro Oferta baseados no funil.`
      ...
      doc.setFont('helvetica', 'bold');
      doc.text("Diretoria Euro Oferta", pageWidth - 58, 247);
      doc.setFont('helvetica', 'normal');
      ...
      <div ref={reportRef} id="printable-report" className="bg-white p-4 sm:p-6 print:p-0 print:bg-white rounded-2xl border border-gray-100/40">
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div>
                  <h1 className="text-xs font-black text-slate-900 uppercase tracking-widest leading-none">Euro Oferta</h1>
                              ...
                              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                                {getSubRowLeads(dev.name, activeExpanded.type).map((lead: Lead) => {
                                  ...
                                  const rawText = activeExpanded.type === 'abertas' 
                                    ? `Olá, ${lead.name}! Sou o ${dev.name} da Euro Oferta. Estou verificando suas preferências da Coleção Premium e gostaria de saber se ficou alguma dúvida com seu pedido. Posso lhe liberar o Cupom ESPECIALVIP com Frete Gratuito e 10% OFF para fecharmos hoje?`
                                    : `Olá, ${lead.name}! Aqui é o ${dev.name} da Euro Oferta. Notamos que sua última negociação foi encerrada recentemente. Gostaríamos muito de entender a sua experiência e oferecer uma condição extraordinária de reativação com 10% de cashback se você ainda tiver interesse! O que acha?`;
        ...
        <div className={`mt-8 pt-4 border-t border-slate-100/80 flex items-center justify-between text-slate-400 font-medium ${exporting ? 'flex' : 'hidden print:flex'}`}>
          <div className="flex items-center gap-1.5 text-[9px] uppercase tracking-wider font-extrabold">
            <span>Euro Oferta Inteligência Comercial</span>
            <span>•</span>
