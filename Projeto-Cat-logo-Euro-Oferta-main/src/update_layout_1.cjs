...
const replace1_search = `              {quickViewProduct.colors && quickViewProduct.colors.length > 0 && (
...
const replacement1 = `              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 sm:gap-4 mt-2 mb-6 border border-gray-100 py-4 px-4 bg-[#FAFAFA] rounded-2xl shadow-[inset_0_1px_4px_rgba(0,0,0,0.02)]">
...
if(code.includes(replace1_search)) {
    code = code.replace(replace1_search, replacement1);
    console.log('Successfully replaced FIRST modal section!');
} else {
    console.log('Could not find first section in text exactly.');
}
