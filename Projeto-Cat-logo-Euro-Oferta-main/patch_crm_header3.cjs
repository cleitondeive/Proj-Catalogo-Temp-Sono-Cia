...
const replace3 = `          <div className="flex flex-wrap items-center gap-3 w-[100%] md:w-auto flex-1 md:flex-none">
...
const replace5 = `            className="h-[42px] px-4 py-2.5 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors shadow-sm flex items-center gap-2 font-bold text-gray-700 shrink-0 whitespace-nowrap"
            title="Exportar Leads (CSV)"
          >
      ...
      <div className="px-5 sm:px-8 pb-5 lg:-mt-2">
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50/50 border border-blue-100 rounded-xl p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between shadow-[0_2px_10px_rgba(59,130,246,0.05)]">
          <div className="flex items-center gap-3 w-full">
             ...
             <div>
               ...
               <p className="text-[13px] font-medium text-[#0F172A] leading-snug lg:leading-normal">
                 Você tem <span className="font-bold underline decoration-blue-200 underline-offset-2">{data.leads.filter((l: any) => !l.nextFollowUp && ['Novo Lead','Em Negociação'].includes(l.status)).length} leads</span> abertos sem follow-up marcado. 
                 Recomendamos priorizar {data.leads.filter((l: any) => l.totalSpent > 0).length} perfis VIPs com alta intenção de compra.
               </p>
             </div>
          </div>
        </div>
...
const returnFilterTarget = `return (nameMatch || phoneMatch) && filterMatch;`;
const returnFilterNew = `return (nameMatch || phoneMatch || emailMatch) && filterMatch;`;
crm = crm.replace(returnFilterTarget, returnFilterNew);
